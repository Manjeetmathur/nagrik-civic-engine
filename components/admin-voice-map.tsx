...
export function AdminVoiceMap({
  ...
  `,
          ...
          <div style="min-width: 200px; color: #18181b; font-family: sans-serif;">
            <b style="font-size: 14px;">${r.keyword}</b><br/>
            <span style="color: #71717a; font-size: 13px;">${r.description}</span><br/>
            <small style="color: #a1a1aa; margin-top: 4px; display: block;">${new Date(r.createdAt).toLocaleString()}</small>
        `
                ...
                if (r.speechStressData) {
                    ...
                    const confidenceColor = r.speechStressData.confidence >= 60 ? '#dc2626' : r.speechStressData.confidence >= 40 ? '#ca8a04' : '#16a34a'

                    popupContent += `
            <hr style="margin: 8px 0; border-color: #e4e4e7;"/>
            <div style="font-size: 12px;">
              <strong>Speech Analysis:</strong><br/>
              <span style="color: ${confidenceColor}; font-weight: 600;">Confidence: ${r.speechStressData.confidence}%</span><br/>
          ...
          `
                    ...
                    if (stressIndicators.length > 0) {
                        popupContent += `<small style="color: #ef4444;">${stressIndicators.slice(0, 2).join(', ')}</small>`
                    }
